...
- 2026-05-18 · The `feat/production-deploy-config-hardening` audit found that `Config::validate_auth` only checked `auth.session_signing_key_b64.is_some()` — it never decoded the value. Because `deploy/relayterm.env.example` ships a recognisable placeholder (`RELAYTERM_AUTH__SESSION_SIGNING_KEY_B64=CHANGE_ME_base64_32_bytes`) AND the v1 hashed-opaque-token session model does NOT consume the signing key, an operator who copied the example file unchanged would boot production with the literal `CHANGE_ME_base64_32_bytes` text as their "signing key" without any boot-time complaint. The hazard is silent: it isn't exploitable today (no consumer), but the moment a future signed-cookie / signed-CSRF slice starts reading the key, every deploy left at the placeholder is silently compromised. The vault key path (`vault_master_key`) and the recording key path catch the same placeholder structurally via `BASE64_STANDARD.decode` (the placeholder's `_` characters aren't standard base64), but the operator-visible error there is a cryptic "invalid byte 0x5F at offset N", not "you left the example placeholder". · Whenever a config field is "reserved but not consumed yet" in production mode, add a defence-in-depth validator that scans the BODY of the value (not just `is_some()`) for known foot-gun shapes — at minimum the matching `CHANGE_ME_*` placeholder substring from `relayterm.env.example`. The canonical helper is `Config::validate_production_secrets()` (apps/backend/src/config.rs), wired from `main.rs` after `validate_auth`. Tests pin the refusal field-by-field (`auth_mode_production_refuses_placeholder_*`) AND pin that error messages never echo the placeholder-bearing value (`validate_production_secrets_errors_do_not_echo_secret_values`). When adding a new production secret-shaped env var, the slice landing it MUST extend this validator AND the example file's "Production-mode boot refusal" header AND `docs/v1-release-checklist.md` § 4 boot-refusal callout in lockstep — otherwise the next placeholder-bypass gap reopens silently.
