...
**Status (this slice).** Design landed (step 1a), typed-config foundation landed (step 1b), AND schema + repository foundation landed (step 2 — `terminal_recording_chunks` and `terminal_recording_markers` migrations under `apps/backend/migrations/`, domain types in `relayterm-core::terminal_recording`, the `TerminalRecordingRepository` trait in `relayterm-core::repository`, the Postgres implementation reachable as `Db::terminal_recordings()`, and the redaction + invariant tests). Application code does NOT yet write recording rows — there is no PTY-forwarder writer, no replay API, no replay-only frontend, no startup reconciliation, no retention worker, no encryption / compression implementation, no `recording_purged` audit kind, no VT snapshot observer, and no export endpoint. The load-bearing invariants below describe the recording subsystem's full target shape — implementation slices 3–9 (`docs/terminal-recording.md` Section 13) are still ahead, and flipping `terminal_recording.enabled = true` today does not record anything.
