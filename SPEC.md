...
**Status today (load-bearing — read before adding any auth code).** Real cookie-backed authentication is wired up across every protected `/api/v1/*` route, but production-auth enablement still fail-fasts at boot until the frontend SPA picks up the new login / bootstrap surface (step 9) and the dev-auth shim is retired (step 10). The API surface is gated by the cookie-backed `AuthenticatedUser` extractor (`crates/relayterm-api/src/auth/user.rs`); a request without a valid `relayterm_session` cookie collapses to a static `401 unauthorized`. State-changing browser-write routes additionally take the shared `_csrf: CsrfGuard` extractor (`crates/relayterm-api/src/auth/csrf.rs`) so a missing or non-allowlisted `Origin` rejects with `403 csrf_origin_mismatch` BEFORE the body is parsed and BEFORE any DB / auth / lifecycle work runs. The legacy `DevUser` extractor and `AppState::dev_user_id` field are still in the codebase but are no longer reachable from any production route handler — they are slated for deletion in step 10 (DevUser retirement). The `users` table carries `id`, `email`, `display_name`, `created_at`, `last_login_at`. The `audit_events_kind_chk` CHECK lists every auth event kind needed today (`first_user_created`, `login_succeeded`, `login_failed`, `logout_succeeded`, `password_changed`, `session_revoked`). As of step 1 below, the typed `AuthConfig` / `AuthMode` plumbing has landed: `auth.mode = dev` is the default and preserves the existing `DevUser` flow, while `auth.mode = production` is rejected at boot via `Config::validate_auth` (production-auth routes, sessions, and password verification have not been implemented yet — failing fast here is what prevents a deploy from silently running without auth). As of step 2 below, the password-credential and browser-session schema, domain types, and Postgres repositories have landed: tables `user_passwords` and `user_sessions` exist; `relayterm-core` exposes `PasswordCredential`, `UserSession`, `UserSessionId`, `CreatePasswordCredential`, `CreateUserSession`, and the `PasswordCredentialRepository` / `UserSessionRepository` traits; `relayterm-db` ships the matching Pg implementations and `Db::password_credentials()` / `Db::user_sessions()` constructors. As of step 3 below, the service-level auth primitives have landed in `relayterm-auth`: `PasswordHasher` (Argon2id with the OWASP 2023 baseline), `SessionToken` / `SessionTokenHash` (32 random bytes from `OsRng`, URL-safe-base64 cookie value, SHA-256 stored digest), and `AuthService` (`set_password`, `verify_password`, `create_session`, `validate_session_token`, `revoke_session`, `revoke_all_for_user`). The legacy `AuthError::NotImplemented` placeholder is retained as a non-breaking shim alongside the new `AuthServiceError`. As of step 4 below, the first auth HTTP surface has landed: `POST /api/v1/auth/bootstrap`, `POST /api/v1/auth/login`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/me`. The login route mints a `relayterm_session` cookie (`HttpOnly; SameSite=Strict; Path=/; Max-Age=2592000`; `Secure` when `auth.cookie_secure = true`) and the logout route clears it; the auth service is reachable through these four routes only. State-changing auth routes carry an inline `Origin` guard against `auth.allowed_origins` (a missing or non-allowlisted origin returns `403 csrf_origin_mismatch`); `GET /auth/me` is exempt. Existing `/api/v1/{hosts,ssh-identities,server-profiles,terminal-sessions,audit-events}` routes still take the `DevUser` extractor; `auth.mode = production` continues to fail fast at boot until the `AuthenticatedUser` extractor migration and shared CSRF middleware land in steps 5–7. The audit-event surface emits `first_user_created` (NEW kind, paired migration `20260501000015`), `login_succeeded`, `login_failed`, and `logout_succeeded`; payloads carry public metadata only and the sentinel-string redaction tests in `crates/relayterm-api/tests/api.rs` pin that no `password` / `password_hash` / `session_token` / `token_hash` / `bootstrap_token` / `argon2id` value reaches a persisted row. As of step 5 below, the cookie-backed `AuthenticatedUser` extractor has landed in `crates/relayterm-api/src/auth/` (`auth/cookie.rs` + `auth/user.rs`); it parses the `relayterm_session` cookie with an exact-match policy (prefix-confusion-safe), validates the token through `AuthService`, and loads the `User` row. **`GET /api/v1/auth/me` is the only consumer today** — every other protected app route still takes `DevUser`. `last_seen_at` is intentionally NOT touched by the extractor in this slice; it lands with the route-migration slice. As of step 6 below, the shared CSRF / `Origin` guard foundation has landed in `crates/relayterm-api/src/auth/csrf.rs` (helper `check_origin(&HeaderMap, &[String])` + axum extractor `CsrfGuard`); the four `/api/v1/auth/*` routes now consume the shared `CsrfGuard` extractor (placed AHEAD of `Json<...>` so the rejection happens before request bytes are parsed and before any DB or auth work runs) and the per-route `AuthRoutesConfig::check_origin` helper is gone. The wire policy is unchanged from step 4 (missing / non-UTF-8 / non-allowlisted `Origin` → `403 csrf_origin_mismatch`; `GET /auth/me` exempt; empty `allowed_origins` rejects every write). The shared guard is the call surface every browser-write route picks up during step 7 (route migration); no existing `DevUser`-protected route consumes it yet. Production-auth enablement still fail-fasts at boot until step 7 (route migration) lands.
...
- **New extractor: `AuthenticatedUser`.** ✅ **Landed.** Lives in `crates/relayterm-api/src/auth/user.rs` (the HTTP-layer glue stays in `relayterm-api`; the crypto and persistence primitives live in `relayterm-auth`, and a thin shared cookie helper lives at `relayterm-api::auth::cookie`). The handler-facing surface is:
      ...
      AppState: FromRef<S>,
  {
      ...
      async fn from_request_parts(parts: &mut Parts, state: &S) -> Result<Self, Self::Rejection> {
          ...
          // 4. `last_seen_at` is INTENTIONALLY NOT touched in this slice
          //    — the pinned approach (managed `JoinSet`, error-tolerant)
          //    lands with the route-migration slice. Pinned here so a
          //    future drop is a deliberate decision and not a regression.
...
- **Transition strategy (two-phase, no big-bang).**
  1. **Phase A — coexistence.** ✅ **Landed.** `AuthenticatedUser` ships AND `DevUser` is unchanged. Both extractors live in the codebase. The only handler that consumes `AuthenticatedUser` today is `GET /api/v1/auth/me` (Phase B will migrate it to a route-by-route cutover). `dev_auth.enabled` defaults stay `true`. The auth crate, schema migrations, and login/logout/bootstrap routes have all already landed; this phase adds the extractor without touching any other protected route. Reverting a misbehaving extractor in this slice means re-pointing `/auth/me` back to its inline implementation — no schema rollback required.
...
1. **Auth-mode + config plumbing.** ✅ **Landed.** `apps/backend/src/config.rs` now carries `AuthConfig { mode, session_signing_key_b64, session_signing_key_file, first_user_bootstrap_token, cookie_secure, cookie_domain, allowed_origins }` with the same `Debug`-redaction posture as `VaultConfig` / `FileVaultConfig` (the secret-shaped fields render as `_set: bool` markers; `FileAuthConfig` mirrors the redaction so the deserialized intermediate cannot re-introduce the leak). `Config::validate_auth` runs in `apps/backend/src/main.rs` BEFORE any irreversible work (db connect, ssh services, listener bind), enforcing: `auth.mode = dev` → OK (regardless of `dev_auth.enabled`); `auth.mode = production` AND `dev_auth.enabled = true` → reject with the explicit mutual-exclusion error; `auth.mode = production` (any `dev_auth.enabled`) → reject with "production auth is not implemented in this build yet" until later steps land. The default mode is `dev`. Reserved keys are read from `RELAYTERM_AUTH__MODE`, `RELAYTERM_AUTH__SESSION_SIGNING_KEY_B64`, `RELAYTERM_AUTH__SESSION_SIGNING_KEY_FILE`, `RELAYTERM_AUTH__FIRST_USER_BOOTSTRAP_TOKEN`, `RELAYTERM_AUTH__COOKIE_SECURE`, `RELAYTERM_AUTH__COOKIE_DOMAIN`, and `RELAYTERM_AUTH__ALLOWED_ORIGINS` (comma-separated). The `[auth]` TOML section mirrors the same names. Unknown `auth.mode` values are rejected at parse time (TOML via serde rename, env via `AuthMode::from_str`) — never silently coerced. The `DevUser` shim and `dev_user_id` flow are unchanged in dev mode. Property-1 tests landed in the same module as the existing vault redaction tests; tests covering the unimplemented-production / mutually-exclusive flag paths pin `validate_auth` against future drift. No routes, migrations, or extractor changes in this step.
...
3. **Auth service.** ✅ **Landed (service primitives only — no routes).** The repository traits and sqlx impls (`relayterm-core::repository::{PasswordCredentialRepository, UserSessionRepository}` + `relayterm-db::{PgPasswordCredentialRepository, PgUserSessionRepository}`) already landed in step 2. This step adds `relayterm-auth::AuthService` plus the password and session-token primitives it composes; no HTTP routes, cookie wiring, CSRF middleware, or extractor changes ship in this slice (those land in steps 4–6).
   ...
   - **What this slice intentionally does NOT do.** No `bootstrap` / `login` / `logout` / `me` routes; no cookie reading or writing; no CSRF middleware; no `AuthenticatedUser` extractor; no route migration off `DevUser`; no frontend auth UI; no passkeys; no production-auth enablement; no audit-event emission of `login_succeeded` / `login_failed` / `logout_succeeded` / `password_changed` / `session_revoked` / `first_user_created`. The audit-kind extension migration is still paired with step 4. Property 8 (logout invalidates at the wire layer) and properties 4, 5, 9, 10, 11, 12 are still route-level tests deferred to later steps.
4. **Login / logout / bootstrap / me API plus inline CSRF guard.** ✅ **Landed.** `POST /api/v1/auth/bootstrap`, `POST /api/v1/auth/login`, `POST /api/v1/auth/logout`, `GET /api/v1/auth/me`. Cookie set / clear in `axum`. Audit-event emission. **Because step 6 has not landed yet, every state-changing auth route carries an INLINE Origin-header check (the small per-route helper [`AuthRoutesConfig::check_origin`], not shared middleware) so login/logout/bootstrap are not CSRF-vulnerable in the gap between steps 4 and 6.** When step 6 lands, the inline check is removed in the same commit that wires the shared middleware so there is no gap and no double-check.
   ...
   - **DevUser interaction.** Existing `/api/v1/{hosts,ssh-identities,server-profiles,terminal-sessions,audit-events}` routes are unchanged in this slice; they still take the `DevUser` extractor. The `AuthenticatedUser` extractor migration is SPEC step 5; until then the auth-routes module lives alongside the shim and the auth service is reachable only by the four new routes.
   - **Production-auth enablement.** Still fail-fast at boot. `auth.mode = production` continues to be rejected by `Config::validate_auth` because the route migration (step 7) and the production-CSRF middleware (step 6) have not landed yet — flipping the gate now would silently leave existing handlers running through `DevUser` even with `dev_auth.enabled = false`.
...
5. **`AuthenticatedUser` extractor.** ✅ **Landed (Phase A — coexistence).** `crates/relayterm-api/src/auth/` now ships the cookie-backed `AuthenticatedUser` extractor (`auth/user.rs`) plus a shared cookie helper (`auth/cookie.rs`) consumed both by the extractor and by the `/api/v1/auth/*` routes. The extractor parses the `Cookie:` header (exact-match on `relayterm_session`; missing / non-UTF-8 / empty-value / prefix-named / suffix-named cookies all collapse to a single 401 indistinguishable from "no cookie"), hashes the token via `relayterm_auth::hash_session_token`, validates it through `AuthService::validate_session_token` (revoked → 401, expired → 401, unknown → 401), then loads the `User` row by id (missing → 401). Failures collapse on the wire to the static `unauthorized` envelope; operator-side detail (`missing session cookie` / `session invalid` / `session expired` / `session revoked` / `session references missing user`) survives in the `warn!` line in `error.rs::IntoResponse`. The handler surface exposes `user_id() -> UserId`, `user() -> &User`, and `into_user() -> User`. The session token, the token hash, and the session row are NEVER reached by the handler — only the resolved `UserId` and `User` are. **Only `GET /api/v1/auth/me` consumes the extractor today; the remaining protected app routes (`hosts`, `server-profiles`, `ssh-identities`, `terminal-sessions`, `audit-events`) still take `DevUser`.** Production-auth enablement still fail-fasts at boot — flipping the gate is part of the route-migration slice (step 7) so the change-blast-radius stays small. **`last_seen_at` is intentionally NOT touched by the extractor in this slice.** The pinned approach (best-effort, error-tolerant, behind a managed `JoinSet` per the AGENTS.md concurrency rule) lands with the route-migration slice when there is a meaningful set of authenticated handlers to amortise the work over; pinning the deferral here so a future `last_seen_at` regression is a deliberate decision and not an accidental drop. Tests: `auth::cookie::tests` pins the parser's exact-match policy across single / multiple / prefix-named / suffix-named / empty-value / non-UTF-8 / no-equals / duplicate / whitespace-padded fixtures (11 unit cases). Postgres-backed integration tests in `crates/relayterm-api/tests/api.rs` cover: `me` returns the user for a valid cookie via the extractor, `me` rejects a missing cookie (401), `me` rejects an unknown cookie (401), `me` rejects an expired session (row inserted with `expires_at` in the past; 401), `me` rejects a revoked session (revoked at the repository; 401), `me` rejects a prefix-confusion cookie (`relayterm_session_other=<real-token>`; 401), the `/me` 200 response carries no `password` / `password_hash` / `session_token` / `token_hash` / `bootstrap_token` / `argon2id` substring AND no sentinel-shaped string from the test password / token / bootstrap secret, and the `DevUser`-protected `GET /api/v1/hosts` route still returns 200 in dev mode (proving the migration has not happened and `DevUser` remains live). The auth crate graduated from `NotImplemented` in step 3.
6. **Shared CSRF / `Origin` guard foundation.** ✅ **Landed.** `crates/relayterm-api/src/auth/csrf.rs` ships the shared helper [`check_origin(&HeaderMap, &[String]) -> Result<(), ApiError>`] and the [`CsrfGuard`] axum extractor (`FromRequestParts`) that wraps it. `AuthRoutesConfig::check_origin` is gone; `POST /api/v1/auth/{bootstrap,login,logout}` now take `_csrf: CsrfGuard` as their first extractor — placed ahead of `Json<...>` so the rejection happens before request bytes are parsed and before any DB or auth work runs. (Ordering note: axum 0.8 runs every `FromRequestParts` extractor before the single `FromRequest` body extractor regardless of source order, so the rejection-before-body-parse guarantee is enforced by axum's invariant; the "ahead of `Json<...>`" placement is convention that keeps the call site self-explanatory and is pinned by the integration test below — not a load-bearing source-order requirement.) The wire policy is unchanged from step 4: missing / non-UTF-8 / non-allowlisted `Origin` → `403 forbidden { code: "csrf_origin_mismatch" }`; empty `allowed_origins` rejects every write; `GET /auth/me` is exempt. The wrapped operator-side detail strings (`missing Origin header` / `Origin header is not valid UTF-8` / `Origin not in allowed_origins`) are deliberately classified — they never echo the offered `Origin` value. Comparison is **case-sensitive byte equality**; a case-insensitive variant is deferred (handling internationalised hostnames safely is its own slice). **What this slice intentionally does NOT do.** No double-submit token (deferred until a non-same-origin client lands per "CSRF posture"); no route-wide `tower` middleware (the extractor approach gives per-route scope without a global allow-list of GET routes); no migration of the existing `DevUser`-protected app routes (that is step 7, and `CsrfGuard` will be added to each browser-write route in the same commit as its `AuthenticatedUser` switch). Production-auth enablement still fail-fasts at boot. **Tests.** `auth::csrf::tests` pins ten unit cases (allow / deny / missing / empty-allowlist / non-UTF-8 / case-sensitivity / trailing-slash / multi-origin allow / two distinct sentinel-Origin redaction cases). Postgres-backed integration tests in `crates/relayterm-api/tests/api.rs` cover: bad Origin rejects BEFORE body parsing (a malformed JSON body paired with a disallowed Origin returns 403 not 400); a CSRF-rejected login does NOT write a `login_failed` audit row (no auth work runs); a CSRF-rejected bootstrap creates no user row AND emits zero auth audit rows; `auth_post_routes_reject_missing_origin`, `auth_post_routes_reject_disallowed_origin`, and `me_does_not_require_origin` continue to pass against the new extractor. Property 5 (CSRF rejects bad Origin) is covered at this layer for the four auth routes; the remaining property 5 coverage lands route-by-route during step 7.
7. **Route migration (Phase B).** ✅ **Landed.** Every protected `/api/v1/*` app route now takes `AuthenticatedUser` instead of `DevUser`. The migrated surfaces are: `hosts` (create/list/get), `ssh-identities` (create/list/get), `server-profiles` (create/list/get + disable/enable + host-key-preflight + trust-host-key + auth-check), `terminal-sessions` (create/list/get/close + the WebSocket attach route), and `audit-events/recent`. Browser-write routes (`POST` / state-changing handlers) additionally take the shared `_csrf: CsrfGuard` extractor as their first parameter so a missing or non-allowlisted `Origin` header rejects with `403 csrf_origin_mismatch` BEFORE the body is parsed AND BEFORE any DB / auth / lifecycle work runs. The WebSocket attach route is `GET` and therefore exempt from `CsrfGuard`; its auth gate is the cookie-backed `AuthenticatedUser` extractor which short-circuits BEFORE the upgrade handshake completes (clients see a clean HTTP 401, not an opened-then-closed socket). Ownership filtering shape is unchanged: handlers extract `UserId` via `user.user_id()`, repository queries stay scoped to `owner_id = caller`, foreign-vs-missing collapses to a byte-identical 404 the same way it did under `DevUser`. The `into_create(owner_id: UserId)` DTO methods on `CreateHostRequest` / `CreateServerProfileRequest` no longer reference `DevUser` — they take a bare `UserId`. Audit lifecycle helpers (`write_lifecycle_audit`, `resolve_owned_profile`) take a `UserId` directly. The `DevUser` extractor module (`crates/relayterm-api/src/dev_user.rs`) and `AppState::dev_user_id` are still in the codebase but are no longer reachable from any production route handler — both will be deleted in step 10 (DevUser retirement) once frontend auth UI lands. Production-auth enablement still fail-fasts at boot until step 9 ships frontend auth UI; flipping the gate now would lock the operator out of the SPA. Tests: every existing test fixture (`setup`, `setup_with_probe`, `setup_full`, `setup_with_auth_check_service`, `setup_with_full_state`, `setup_with_full_state_short_ttl`, `setup_with_fake_probe`, `setup_with_fake_auth_checker`, `setup_with_pty_bridge`) was updated to bootstrap a real `AuthService` session via `bootstrap_test_session(&auth, user_id)` and return the cookie-token plaintext as the last tuple element; `dev_user_id` is set to `None` on every fixture so an accidental `DevUser` regression in a route would surface as 401 instead of silently passing. The `json_post(uri, body, cookie)` and `get(uri, cookie)` request builders attach the cookie + Origin (POST only) automatically; `json_post_no_auth` / `get_no_auth` cover the missing-cookie 401 paths; `json_post_with_origin(uri, body, cookie, origin)` covers the bad-Origin 403 paths. WebSocket helpers (`open_ws`, `open_ws_attached`, `ws_handshake_status`) take the cookie token and attach it to the upgrade handshake. New integration tests cover: `protected_hosts_routes_return_401_without_session_cookie` (GET + POST `/hosts` reject when no cookie), `post_ssh_identity_returns_401_without_session_cookie`, `auth_check_returns_401_without_session_cookie`, `terminal_session_routes_return_401_without_session_cookie` (covers create / list / get / close on the v1 surface), `ws_attach_returns_401_without_session_cookie` (the WebSocket handshake fails BEFORE upgrade), `audit_events_recent_unauthorized_without_session_cookie`, `create_host_bad_origin_returns_403_before_body_parse` (malformed JSON body + disallowed Origin → 403, not 400; no row written), `create_host_missing_origin_returns_403`, and `disable_with_bad_origin_returns_403_and_writes_no_audit` (lifecycle audit row count stays 0 on the bad-Origin path). Property 5 (CSRF rejects bad Origin) is now covered for at least one representative route per surface; property 7 (cross-user 404 indistinguishable) and property 10 (dev shim cannot stamp a request when no cookie is present) are exercised against the new extractor. Production-auth enablement is **NOT** flipped in this slice — `Config::validate_auth` continues to reject `auth.mode = production` until frontend auth UI ships (step 9) AND DevUser retirement ships (step 10).
...
9. **Frontend auth phases 1–6.** ✅ **Landed (foundation — phases 1–4; phases 5–6 still deferred).** `apps/web/src/lib/api/auth.ts` ships typed helpers for `getCurrentUser`, `login`, `logout`, and `bootstrap` plus the field-by-field `parseCurrentUser` parser, the `describeAuthError` formatter (function of `kind` + `status` + `code` only), and frontend-side `validateLoginForm` / `validateBootstrapForm` mirrors of the backend bounds. Every helper sets `credentials: "include"` so the browser ships the `relayterm_session` cookie; nothing in the SPA reads, writes, or echoes the cookie value. The `Origin` header is never set from JS — the browser controls it on POSTs and the backend's CSRF guard is appeased by the browser-attached value. `apps/web/src/lib/app/auth/AuthGate.svelte` mounts at the top of `App.svelte` (above both the dev-mode and prod branches), calls `getCurrentUser()` on mount, and short-circuits the rest of the SPA: a small `auth-loading` splash while in flight, `auth-error-screen` (with explicit retry; no auto-retry storm) on transport / 5xx / malformed, `LoginView` on HTTP 401, and the existing `AppShell` view tree on a parsed user. `LoginView` (`auth-login-*` selectors) submits to `POST /api/v1/auth/login` and collapses the wire 401 to a generic "invalid credentials" line — the copy never reveals whether the offered email belongs to a known account. A "First-time setup" affordance switches the unauthenticated screen to `BootstrapView` (`auth-bootstrap-*` selectors); bootstrap creates the user, shows "Account created. Please sign in.", and routes back to `LoginView` (no auto-login — keeping session minting on the login route only, per "Frontend authentication UI plan" Phase 3). The `TopBar`'s `auth-sign-out` button calls `POST /api/v1/auth/logout` and ALWAYS runs local cleanup afterwards (clears the active-terminal pointer and `activeLaunch`, drops the gate to the login screen) regardless of the wire outcome — a flaky network can never trap an operator in a logged-in UI state. The bootstrap form's `bootstrap_token` is a `<input type="password">`; the SPA does not persist the token, the password, the session token, or any decoded session payload to local storage. The redaction posture is sentinel-tested in `apps/web/tests/authApi.test.ts` (42 cases): `parseCurrentUser` drops smuggled `password_hash` / `session_token` / `token_hash` / `bootstrap_token` / `private_key` / `encrypted_private_key` / `access_token` / `session_output` field-by-field; `describeAuthError` never echoes the wire `message` or transport detail; `login` / `bootstrap` request inputs (offered password / bootstrap token) never reach an error string or `console.*`. **Discovery cutover.** The first slice does NOT yet teach the SPA to discover whether bootstrap is currently enabled — the `getCurrentUser()` 401 path drops to `LoginView` in every case, and the operator clicks "First-time setup" knowing whether the backend has a token configured. The 503 from the bootstrap route is the wire-side rejection; the 409 collapses to "an account already exists." A future slice can add a probe route or an explicit signal once we decide what shape the discovery path should take. **Phases still deferred.** Phase 5 (a `lib/api/authState.ts` store + a `fetchJson` 401 interceptor that drops the SPA to `LoginView` from any protected `/api/v1/*` 401) is NOT in this slice — protected views still surface their own 401 via the per-view error formatter, which is acceptable until we have a richer story for "session expired mid-flow." Phase 6 (URL-based route guarding that preserves the originally-requested path across login) is also deferred — `AuthGate` short-circuits the entire view tree when unauthenticated, so the gate IS a guard; the "preserve and restore the requested path" affordance is its own slice. Production-auth enablement still fail-fasts at boot: `Config::validate_auth` continues to reject `auth.mode = production` until step 10 (DevUser retirement) ships AND a deployment-time decision is made about CSRF / cookie config. The frontend now MOSTLY satisfies the prerequisite for that flip; the remaining gap is operational, not code.
