...
**Status (this slice).** Design landed (step 1a), typed-config foundation landed (step 1b), schema + repository foundation landed (step 2), the orchestrator's recording-writer foundation landed (step 3 — partial), the durable replay read API foundation landed (step 4 — partial), AND the frontend replay viewer foundation has now landed (step 5 — partial). The frontend slice adds a read-only replay viewer (`apps/web/src/lib/app/views/RecordingReplayView.svelte`) reachable from the Sessions list via a new `View recording` action. The viewer fetches metadata, pages chunks page-by-page, and writes decoded bytes directly into a read-only `XtermRenderer` — keyboard input is disabled (`xtermOnly: { disableStdin: true }`) and there is no input listener, no live SSH client, and no WebSocket attach. Decoded chunk bytes are NEVER stashed in `$state`, `localStorage`, or `sessionStorage`; the viewer holds them only long enough to call `renderer.write(...)` per chunk. Markers are rendered as a metadata strip only (key/value preview), never as terminal output. A clear "Replay only" banner names every limitation: input was not recorded, the live SSH session cannot be resumed from a recording, backend-restart recovery is not implemented yet, and a recording may end mid-output if the backend restarted while the session was alive. The Sessions list deliberately does NOT pre-fetch metadata for every row (would be N+1); the affordance is offered for any non-`starting` row and the viewer's metadata gate honestly surfaces "No recording available" when an opened session turns out empty. Three new owner-scoped GET routes — `/api/v1/terminal-sessions/:id/recording/{metadata,chunks,markers}` — surface the persisted recording artefacts. Chunk payloads cross the wire ONLY as base64 (`data_b64`); raw bytes never reach a log line, error path, or audit row. The repository surface is session-scoped (`TerminalRecordingRepository::get_metadata`, `list_chunks`, `list_markers`); owner scoping happens at the route layer via the existing `terminal_sessions.owner_id == user.user_id()` filter, with foreign-owned and missing collapsing to a byte-identical 404. Markers wired in the writer slice are `started`, `closed`, `resized`, and `replay_gap`; `attached` / `detached` / `reattached` are deferred to a follow-up that threads the writer handle through the attach surface. Supported runtime mode is `terminal_recording.enabled = true` AND `encryption.mode = disabled` (plaintext-at-rest) only — the production-required `encryption.mode = required` writer has not landed and the backend FAILS TO BOOT in that combination so an operator cannot silently degrade to plaintext. Recording is OFF by default; flipping it on AND choosing `encryption.mode = disabled` (dev only — operator accepts plaintext-at-rest) is what activates the writer. There is still no live attach fallback to durable chunks, no startup reconciliation, no retention worker, no encryption / compression implementation, no `recording_purged` audit kind, no VT snapshot observer, no export / download endpoint, and no admin / cross-user recording access. Implementation slices 5 (live-attach bridge + viewer polish), 6, 7, 8, 9 (`docs/terminal-recording.md` Section 13) are still ahead.
...
- **Backend-restart reconciliation is idempotent and audited.** On startup, the orchestrator scans `terminal_sessions WHERE status IN ('starting', 'active', 'detached')`, transitions each to `closed` with a `closed { reason: "backend_restart", category: "process_lost" }` lifecycle event, and (if recording rows exist) writes a `closed` recording marker at the highest persisted seq. Reconciliation does NOT delete chunk rows. A second startup that finds no pre-restart non-`closed` rows is a no-op.
...
6. Startup reconciliation for `active` rows after a crash.
