...
8. Retention cleanup job (and a new `recording_purged` audit kind) — **design landed (Section 12 of `docs/terminal-recording.md`); worker, migration, and audit kind not yet implemented.** Eligibility, transactional shape, fail-closed audit, config block (`[terminal_recording.cleanup]`, independent of `terminal_recording.enabled`), staged worker (Stage A startup-only sweep + Stage B periodic managed task with advisory lock and graceful-shutdown), and post-purge wire shape are pinned in Section 12; implementation step 8 lands the migration + `AuditEventKind::RecordingPurged` + `delete_recording_for_session` repository method + cleanup config + the worker.
