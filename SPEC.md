...
- Frontend active-sessions UI (the API foundation — `GET /api/v1/auth/sessions`, `POST /api/v1/auth/sessions/:id/revoke`, `POST /api/v1/auth/sessions/revoke-all-except-current` — is now live; the UI that consumes it is its own slice). Admin / cross-user session view is also out of scope (`/auth/sessions` is current-user only by design).
