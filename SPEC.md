...
**Status (this slice).** Design landed (step 1a), typed-config foundation landed (step 1b), schema + repository foundation landed (step 2), AND the orchestrator's recording-writer foundation landed (step 3 — partial). The `RecordingWriter` and `RecordingRuntime` types in `crates/relayterm-terminal/src/recording.rs` tee PTY OUTPUT bytes from the manager's forwarder into bounded chunk and marker rows; the writer is a tee, never a gate, and never blocks the live wire. Markers wired in this slice are `started`, `closed`, `resized`, and `replay_gap`; `attached` / `detached` / `reattached` are deferred to a follow-up that threads the writer handle through the attach surface. Supported runtime mode in this slice is `terminal_recording.enabled = true` AND `encryption.mode = disabled` (plaintext-at-rest) only — the production-required `encryption.mode = required` writer has not landed and the backend now FAILS TO BOOT in that combination so an operator cannot silently degrade to plaintext. Recording is OFF by default; flipping it on AND choosing `encryption.mode = disabled` (dev only — operator accepts plaintext-at-rest) is what activates the writer. There is still no durable replay API, no replay-only frontend, no startup reconciliation, no retention worker, no encryption / compression implementation, no `recording_purged` audit kind, no VT snapshot observer, and no export endpoint. Implementation slices 4–9 (`docs/terminal-recording.md` Section 13) are still ahead.
