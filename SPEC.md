...
- Backend-restart recovery for live sessions. A restart drops the in-memory runtime registry and the live `russh::Channel` is unrecoverable. The orchestrator now runs a startup reconciliation pass at boot that transitions every `terminal_sessions` row in `starting`/`active`/`detached` to `closed` with a matching `closed { reason: "startup_reconciliation", previous_status, reconciled_at }` `session_events` row written in the same database transaction, so a stale "live" row never reaches the UI; the live PTY itself is not resurrected (recording captures display history, not the live shell). See "Durable terminal recording and replay architecture" for the full policy.
...
**Status (this slice).** Design landed (step 1a), typed-config foundation landed (step 1b), schema + repository foundation landed (step 2), the orchestrator's recording-writer foundation landed (step 3 — partial), the durable replay read API foundation landed (step 4 — partial), the frontend replay viewer foundation landed (step 5 — partial), AND the backend startup reconciliation foundation has now landed (step 6 — partial; `terminal_sessions` + `session_events` only). The startup slice runs a single repository pass at boot — AFTER the database pool is ready and BEFORE the listener binds — that locks every `terminal_sessions WHERE status IN ('starting', 'active', 'detached')` row, transitions each to `closed`, and inside the SAME database transaction appends one matching `session_events { kind: closed, payload: { reason: "startup_reconciliation", previous_status, reconciled_at } }` row per session. The pass writes ZERO `audit_events` rows (matches the existing close-path audit shape; if a future slice decides restart-reconciliation IS audit-worthy, it adds a dedicated `terminal_session_reconciled` audit kind through the standard migration pattern). It does NOT delete any `terminal_sessions` row. It does NOT touch `terminal_recording_chunks` or `terminal_recording_markers` — recordings remain readable through the existing closed-session replay path. It is idempotent: a second boot finds no orphans and writes nothing. A failure to reconcile fails startup before the listener binds; the operator sees the boot abort instead of a backend that serves stale "live" rows the UI cannot resume. The closed-recording-marker write at the highest persisted seq (design-intent in `docs/terminal-recording.md` Section 9.3) is **deferred** to a follow-up slice; the current "Replay only" banner already names "a recording may end mid-output if the backend restarted while the session was alive" so user-facing copy does not change. The frontend slice adds a read-only replay viewer (`apps/web/src/lib/app/views/RecordingReplayView.svelte`) reachable from the Sessions list via a new `View recording` action. The viewer fetches metadata, pages chunks page-by-page, and writes decoded bytes directly into a read-only `XtermRenderer` — keyboard input is disabled (`xtermOnly: { disableStdin: true }`) and there is no input listener, no live SSH client, and no WebSocket attach. Decoded chunk bytes are NEVER stashed in `$state`, `localStorage`, or `sessionStorage`; the viewer holds them only long enough to call `renderer.write(...)` per chunk. Markers are rendered as a metadata strip only (key/value preview), never as terminal output. The Sessions list deliberately does NOT pre-fetch metadata for every row (would be N+1); the affordance is offered for any non-`starting` row and the viewer's metadata gate honestly surfaces "No recording available" when an opened session turns out empty. Three owner-scoped GET routes — `/api/v1/terminal-sessions/:id/recording/{metadata,chunks,markers}` — surface the persisted recording artefacts. Chunk payloads cross the wire ONLY as base64 (`data_b64`); raw bytes never reach a log line, error path, or audit row. The repository surface is session-scoped (`TerminalRecordingRepository::get_metadata`, `list_chunks`, `list_markers`); owner scoping happens at the route layer via the existing `terminal_sessions.owner_id == user.user_id()` filter, with foreign-owned and missing collapsing to a byte-identical 404. Markers wired in the writer slice are `started`, `closed`, `resized`, and `replay_gap`; `attached` / `detached` / `reattached` are deferred to a follow-up that threads the writer handle through the attach surface. Supported runtime mode is `terminal_recording.enabled = true` AND `encryption.mode = disabled` (plaintext-at-rest) only — the production-required `encryption.mode = required` writer has not landed and the backend FAILS TO BOOT in that combination so an operator cannot silently degrade to plaintext. Recording is OFF by default; flipping it on AND choosing `encryption.mode = disabled` (dev only — operator accepts plaintext-at-rest) is what activates the writer. There is still no live attach fallback to durable chunks, no closed-recording-marker write at startup reconciliation, no retention worker, no encryption / compression implementation, no `recording_purged` audit kind, no VT snapshot observer, no export / download endpoint, and no admin / cross-user recording access. Implementation slices 5 (live-attach bridge + viewer polish), the startup-reconciliation marker write follow-up, 7, 8, 9 (`docs/terminal-recording.md` Section 13) are still ahead.
...
- **Backend-restart reconciliation is idempotent and audited.** On startup, the orchestrator scans `terminal_sessions WHERE status IN ('starting', 'active', 'detached')`, transitions each to `closed`, and inside the same database transaction appends one matching `closed` `session_events` row per session with payload `{ "reason": "startup_reconciliation", "previous_status": <starting|active|detached>, "reconciled_at": <UTC timestamp> }`. The pass writes ZERO `audit_events` rows (operational bookkeeping, matches the existing close-path audit shape) and never deletes any session, chunk, or marker row — recordings remain readable through the closed-session replay path. The closed-recording-marker write at the highest persisted seq is design-intent (see `docs/terminal-recording.md` Section 9.3) but is deferred to a follow-up slice. Reconciliation runs BEFORE the HTTP listener binds; a database failure on the reconciliation transaction fails the boot rather than starting a backend that would serve stale "live" rows the UI cannot resume. A second startup that finds no pre-restart non-`closed` rows is a byte-identical no-op (no events appended, no rows touched).
...
6. Startup reconciliation for `active` rows after a crash — foundation landed: a single `terminal_sessions` repository pass at boot transitions any `status IN ('starting', 'active', 'detached')` row to `closed` and appends one matching `closed { reason: "startup_reconciliation", previous_status, reconciled_at }` `session_events` row per session in the same transaction, writes zero `audit_events`, leaves recording chunks/markers untouched, runs before the HTTP listener binds, and is idempotent. The closed-recording-marker write at the highest persisted seq is deferred to a follow-up slice.
