...
In parallel with the renderer-evaluation track above, the concrete cutline for "first version deployable for a single self-hosted operator" — definition, evidence-based blockers, post-v1 deferrals, deployment smoke checklist, and open operator decisions — lives in [`docs/v1-production-readiness.md`](docs/v1-production-readiness.md). The cutline does NOT redefine any contract in this file or in `docs/spec/`; it composes them into a release punch list. Renderer evaluation continues independently of v1 and is explicitly off the v1 critical path.
