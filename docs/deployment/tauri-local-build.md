...
- ✅ Desktop handoff + login + **terminal session attach** smoke (2026-05-09, on CachyOS / Arch with WebKitGTK 4.1) — re-used the existing `target/release/relayterm-desktop` (no rebuild) and verified the bundled shell end-to-end against an isolated, throwaway, local-only Compose stack (separate project namespace, image tag `:main`, throwaway env file outside the repo, throwaway secrets only — never committed, never logged). Bootstrap → login → SSH identity creation → host / server-profile / host-key preflight / trust → terminal session attach all succeeded inside the Tauri WebView post-handoff. Three harmless sentinel commands (`echo relayterm-tauri-smoke`, `whoami`, `pwd`) round-tripped through the WebSocket data plane to a throwaway `linuxserver/openssh-server` container on the same Compose network (key auth only, public key generated by RelayTerm — no real SSH private keys, no production host, sshd not exposed to the host network). Detach + reconnect, end-session, and disconnect all worked. No production credentials used; no production SSH targets touched; backend / auth / CORS / CSRF / Tauri-capability code unchanged. Detail in [`docs/spec/tauri-runtime-backend-url.md`](../spec/tauri-runtime-backend-url.md) "Phase E — verification log". Verifies local-build + path-A handoff + terminal data plane only — does NOT verify production HTTPS, Android terminal attach, long-lived detach, mobile background lifecycle, real production SSH identities, alternate renderers, or terminal recording.
