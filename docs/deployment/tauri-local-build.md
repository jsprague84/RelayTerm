...
- **Backend connectivity is now configurable for the bundled shell** via the phase C bootstrap picker described in [`docs/spec/tauri-runtime-backend-url.md`](../spec/tauri-runtime-backend-url.md). On first launch the built Tauri shell shows a single-field "Connect to RelayTerm Server" form; on save it persists a canonical origin to `localStorage` and navigates the WebView there. The "Connecting…" splash that mounts during handoff also exposes a **"Change server" button** that cancels the pending navigation, clears the persisted origin, and re-renders the picker — operators no longer need to hand-edit `localStorage` (or wipe app data) to pick a different backend. Backend connectivity itself (login, identity list, terminal attach, recordings) is still subject to the configured server actually being reachable — that remains a separate manual smoke and is NOT verified by the launch test above.
...
| `pnpm --filter @relayterm/desktop exec tauri build --bundles deb,rpm` followed by launching `target/release/relayterm-desktop` against a local hosted RelayTerm web origin (built bootstrap picker + remote-web handoff smoke) | ⚠ Partially verified on the same CachyOS host, 2026-05-08, with the Phase C bootstrap picker described in [`docs/spec/tauri-runtime-backend-url.md`](../spec/tauri-runtime-backend-url.md). With no prior app data the picker rendered ("Connect to RelayTerm Server"). Three validator probes passed verbatim against `describeBackendUrlError`: `http://example.com` → `Use https:// for any host other than localhost.`; `https://user:pass@example.com` → `Remove the username/password from the URL — credentials are never part of the server address.`; a valid local hosted RelayTerm web origin was accepted, persisted under `relayterm.backend-config.v1`, and the WebView reloaded at the configured origin via `window.location.assign(${origin}/)`. The post-handoff target rendered the production RelayTerm web shell. Verifies bundled-shell picker render + URL validation + handoff navigation only — does NOT verify login / auth, identity list / terminal attach, the "Change server" runtime click on a built shell (the affordance is shipped on the Connecting splash and pinned by the `Change Server reset flow` tests in [`apps/web/tests/backendHandoff.test.ts`](../../apps/web/tests/backendHandoff.test.ts), but on-device runtime smoke is deferred to a later interactive run), or the Android handoff path. |
| `pnpm --filter @relayterm/desktop exec tauri build --bundles deb,rpm` followed by launching `target/release/relayterm-desktop` against an isolated, throwaway, local-only RelayTerm Compose stack (built bootstrap handoff + login / auth smoke) | ⚠ Partially verified on the same CachyOS host, 2026-05-09. Followed the runtime-backend handoff verified on 2026-05-08, then exercised the bootstrap → login → logout path against a locally hosted RelayTerm web origin served by `deploy/docker-compose.images.example.yml` (separate Compose project namespace; image tag `:main` from the most recent Forgejo publish; `auth.mode = dev`; `cookie_secure = false` for the plain-HTTP loopback origin; throwaway env file outside the repo; throwaway secrets only — never committed, never logged). The Tauri shell auto-handed off to the saved local origin (no picker interaction this run); `AuthGate` rendered `LoginView`; first-user bootstrap succeeded with throwaway token + email + display name + password and routed back to login per design; login returned the operator to the app shell (cookie + `Origin` allow-list flow works same-origin); sign-out flipped the gate back to `LoginView` cleanly. Backend log scan over the auth-path window: zero `csrf_origin_mismatch`, zero `WARN`, zero `ERROR` from WebView-issued requests. **Operator-config caveat:** the saved origin (`http://localhost:8081`) and the env's initial `RELAYTERM_AUTH__ALLOWED_ORIGINS` (`http://127.0.0.1:8081`) are different `Origin` strings; the smoke had to extend allowed-origins to include both forms before bootstrap could pass CSRF — operator-side `.env` adjustment, not a code, CORS, cookie, or CSRF policy change. Verifies the bundled-shell handoff + browser-style auth round trip only — does NOT verify terminal session attach, SSH session launch, in-shell "Change server" runtime click on this run (the saved config was already correct so the picker was bypassed), the Android handoff login path, production hosted smoke, native secure storage, or multi-server support. No real credentials used. See [`docs/spec/tauri-runtime-backend-url.md`](../spec/tauri-runtime-backend-url.md) § "Phase E — verification log" for the design-side log. |
| `pnpm --filter @relayterm/desktop exec tauri build --bundles deb,rpm` followed by launching `target/release/relayterm-desktop` against the same throwaway local Compose stack (handoff stability — same-origin short-circuit landing) | ⚠ Partially verified on the same CachyOS host, 2026-05-09. Smoke originally targeted as a docs-only "terminal attach" branch but uncovered a path A handoff loop instead — the same-origin short-circuit was added to `apps/web/src/lib/runtime/backendHandoff.ts` (returns `{ kind: "passthrough", reason: "already_at_backend" }` when `cfg.backendOrigin === window.location.origin`), pinned by the new `decideHandoff — same-origin short-circuit (already_at_backend)` block in [`apps/web/tests/backendHandoff.test.ts`](../../apps/web/tests/backendHandoff.test.ts), the desktop binary was rebuilt, the served `:main` web bundle was swapped for the freshly-built `apps/web/dist` via `docker cp` + `nginx -s reload`, and the WebKit HTTP cache was wiped (see "WebKit HTTP cache + nginx immutable assets" under Troubleshooting). On relaunch the bundled SPA auto-handed off to `http://localhost:8081`, the WebView arrived at the configured origin, the picker rendered once at the remote origin, the operator saved the same origin, and `LoginView` rendered without any flashing splash or loop. Verifies path A handoff stability when **both** the bundled SPA and the served SPA carry `ConfiguredBackendGate` — does NOT verify terminal session attach (still deferred to a later branch), Android handoff, production HTTPS, "Change server" runtime click on a built shell (the saved config was correct after the test save, so the picker was not re-entered), or any operator-cache state besides the local WebKit cache. No real credentials used. See [`docs/spec/tauri-runtime-backend-url.md`](../spec/tauri-runtime-backend-url.md) § "Phase E — verification log" for the design-side log. |
...
- **Backend connectivity in the built (non-dev) shell.** Phase 0 does **not** wire production backend selection. If the bundled SPA shows network errors, that is expected — see "Deferred work". The design for how a built Tauri shell chooses and persists a backend URL — including the cookie / CSRF / `Origin` constraints that drive the recommended "remote web shell" approach — lives in [`docs/spec/tauri-runtime-backend-url.md`](../spec/tauri-runtime-backend-url.md). The launch-smoke modal `Cannot Reach RelayTerm` / `Cannot reach the backend: malformed response` is the deferred-runtime-backend-URL failure path that doc closes.
- **Stuck on the Connecting splash / saved a wrong backend URL.** The "Change server" button on the Connecting splash (`data-testid="tauri-bootstrap-change-server"`) is the supported reset path: it cancels the pending navigation, clears the persisted origin from `localStorage`, and re-renders the picker. As a fallback (e.g. if the WebView already reloaded and the remote SPA fails to load anything clickable, or for older builds without the affordance), open the WebView's developer tools and run `localStorage.removeItem("relayterm.backend-config.v1")` to wipe the persisted origin, then relaunch. App-data wipe via the OS package manager works too, but takes the rest of the WebView's storage with it.
- **Connecting splash flashes after pressing Connect (path A handoff loop).** A built Tauri shell that hands off to a configured backend whose served SPA also carries `ConfiguredBackendGate` (i.e. any RelayTerm web image of recent vintage — pre-`fix/tauri-handoff-navigate-loop` builds) will run the gate at the post-handoff origin too, because the Tauri v2 WebView injects `__TAURI_INTERNALS__` on remote-origin pages. Without a same-origin short-circuit the gate sees its own (just-saved) config and schedules another `window.location.assign(${origin}/)`, looping the page. Fixed in `apps/web/src/lib/runtime/backendHandoff.ts` by returning `{ kind: "passthrough", reason: "already_at_backend" }` when `cfg.backendOrigin` byte-equals `window.location.origin`; pinned by the `decideHandoff — same-origin short-circuit (already_at_backend)` block in [`apps/web/tests/backendHandoff.test.ts`](../../apps/web/tests/backendHandoff.test.ts). If you observe the flashing splash after a `Connect` press on a current build, the most likely operator-side cause is the **WebKit cache pitfall** below — not the gate logic.
- **WebKit HTTP cache + nginx immutable assets (local-stack iteration only).** When iterating on the desktop SPA against a local Compose stack — e.g. `docker cp apps/web/dist/. <web-container>:/usr/share/nginx/html/` + `nginx -s reload` to swap the served bundle without rebuilding the image — the desktop WebView keeps serving the previously-cached `/assets/index-<hash>.js` from its WebKitGTK HTTP cache rather than re-fetching the new bundle. `deploy/nginx/web.conf.template` sets `Cache-Control: public, immutable` + `max-age=31536000` on `/assets/*` (correct for production, where Vite emits content-hashed filenames so each code change produces a NEW URL), so revalidation is bypassed entirely. Symptom: post-handoff "Connecting…" splash flashes / loops because the WebView is still running the older un-fixed bundle even though `curl http://127.0.0.1:8081/assets/index-<hash>.js` from the host returns the new content. Workaround: `pkill -f target/release/relayterm-desktop`, then `rm -rf ~/.local/share/cc.js-node.relayterm.desktop/{WebKitCache,CacheStorage}` (and the affected origin's localStorage if you need a clean re-pick: `~/.local/share/cc.js-node.relayterm.desktop/localstorage/http_localhost_8081.localstorage*`), then relaunch. Production deployments do NOT hit this — Vite's content-hashed filenames make every code change a different URL, so the cache key changes and the WebView re-fetches naturally; the pitfall is specific to in-place bundle swaps that re-use the same hashed filename. See `docs/spec/tauri-runtime-backend-url.md` § "Phase E — verification log" for the same-origin short-circuit smoke that uncovered this.
...
- **Runtime API base URL / backend selection** for built (non-dev) desktop and mobile shells — no production proxy, no env-driven URL, no in-app picker, no native command bridge for config. The bundled SPA has no Vite dev proxy. Design landed at [`docs/spec/tauri-runtime-backend-url.md`](../spec/tauri-runtime-backend-url.md) (path A — remote web shell — recommended for v1; path B — bundled SPA + cross-origin API — explicitly deferred because it would weaken the production cookie / CSRF posture).
