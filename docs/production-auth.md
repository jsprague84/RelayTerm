...
- **No session-management UI.** `user_sessions.last_seen_at` is now stamped on every successful authenticated request (best-effort inline; a repository failure logs `warn!` and the request still succeeds; failed / expired / revoked / unknown extractions never reach the touch), so the future session-management UI has the data it needs to surface idle / active sessions. The UI itself is deferred — `POST /api/v1/auth/logout` still revokes the *current* session only, and there is no per-session revoke or "log out everywhere" surface yet.
