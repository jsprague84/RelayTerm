...
- The footer note carries the remaining future-work list: passphrase-protected key import, RSA / ECDSA, file pickers, password bootstrap, and `ssh-copy-id` automation are deliberate later slices. (SSH-identity rename and delete have landed — see `apps/web/src/lib/app/views/IdentitiesView.svelte` for the in-place rename + delete affordances; the full route contract is in SPEC.md → "Inventory lifecycle and destructive-action policy". Private-key import has now landed as the sibling "Import SSH identity" panel — see "Production SSH identity import UI (landed)" below.)

**Production SSH identity import UI (landed).**

A second panel on `apps/web/src/lib/app/views/IdentitiesView.svelte` lets the operator paste an existing OpenSSH-format Ed25519 PEM and import it into the same backend vault as a generated identity. Implementation slice: `feat/private-key-import-v1`; full design at [`docs/private-key-import.md`](../private-key-import.md).

- **Affordance.** A second action button "Import SSH identity" sits beside "Generate SSH identity" in the view header (`identities-import-open`). Clicking it opens a panel (`identities-import-panel`); the Generate and Import panels are mutually exclusive — opening one closes the other so at most one form bearing secret material is on screen.
- **Panel form.** A `name` input (`identities-import-name`, mirrors the generate validator byte-for-byte) and a `<textarea>` for the OpenSSH PEM (`identities-import-private-key`, `autocomplete="off"`, `autocapitalize="none"`, `spellcheck="false"`, `inputmode="text"`, `maxlength = 8 KiB`). The submit button (`identities-import-submit`) is disabled while submitting OR while the name is empty after trim OR while the textarea is empty.
- **No file picker, no passphrase field.** v1 ingress is paste-only; encrypted (passphrase-protected) keys are refused server-side with a typed `400 unsupported_key_format encrypted` and surfaced via `describeImportSshIdentityError`.
- **Honesty copy.** The panel intro tells the operator the private key is sent over HTTPS, parsed once, encrypted into the server-side vault, and is NOT stored in the browser, localStorage, or sessionStorage; the textarea is cleared on success and on every failure.
- **PEM handling discipline (load-bearing).** The pasted PEM lives in a script-scoped `pendingImportPrivateKey` `$state` string bound to the textarea. On submit the string is snapshotted into a local variable, then the bound state is cleared BEFORE the fetch await — so the textarea is empty for the rest of the request lifecycle. On every failure branch (validation / HTTP / transport / parse) the textarea remains empty; on panel close (`identities-import-close`) both `name` and `pendingImportPrivateKey` are cleared. The PEM is NEVER persisted to localStorage / sessionStorage / a `data-*` attribute / a store / an audit string / a panel error. The `parseSshIdentity` field-by-field parser remains the redaction backstop on the response side.
- **Success card.** Shows the operator-supplied name, the derived `key_type`, the `fingerprint_sha256`, and the public-key OpenSSH line — every field is public metadata. The same "Copy public key" affordance the generate success card uses, with the same "copies the public key only — never any private material" honesty line.
- **Error mapping.** `describeImportSshIdentityError` formats one-line summaries that are a function of `kind` + `status` + `code` + the derived `reason` enum only — never the wire `message`, never the thrown `Error.message`. Mapped reasons: `duplicate_fingerprint` ("this key is already imported"), `unsupported_format_encrypted` ("passphrase-protected keys are not supported in this release"), `unsupported_format_malformed` ("not a valid OpenSSH private key"), `unsupported_format_not_a_private_key` (reserved), `unsupported_key_type` ("only OpenSSH-format Ed25519 private keys are supported"). 401 / 403 / 503 collapse to the standard auth / browser-policy / vault-disabled summaries.
- **Audit pairing.** Both the generate route and the import route now emit one `ssh_identity_created` row per success, distinguished by `source: "generated" | "imported"` in the public-only payload (`ssh_identity_id`, `name`, `key_type`, `fingerprint_sha256`, `created_at`, `source`). The duplicate-fingerprint 409 short-circuits BEFORE the audit append so a duplicate-paste retry cannot accumulate audit rows.

**Future work (explicit out-of-scope for this slice).**

Passphrase-protected (encrypted) OpenSSH-key import — design § 1 / § 13.1 reserves it for v1.1 with a one-shot HTTPS-only passphrase channel that pulls in `ssh-key`'s `encryption` cipher feature. RSA / ECDSA / DSA import; PEM PKCS#1 / PKCS#8 import; Putty `.ppk`; file picker; bulk import; hardware / FIDO / U2F / smart-card SSH keys; SSH certificates (CA-signed); admin / cross-user import; key-rotation workflow; `ssh-copy-id` / password-bootstrap automation. Each is a separate slice.
