...
**`focusTarget()` status (load-bearing).** `WtermRenderer` now implements the optional `focusTarget()` method on the neutral `TerminalRenderer` interface (landed in `feat/wterm-renderer-focus-target`). It returns wterm's hidden keyboard `<textarea>` — the `InputHandler.textarea` element wterm appends to the host, attaches its `keydown` listener to, and the one `WTerm.focus()` ultimately focuses — and `null` before mount / after dispose / after a dispose that raced a pending `init()`. The adapter reaches that element through a narrow structural cast over wterm's type-level-private `input` / `textarea` fields, contained in the adapter (the only file in the repo that knows wterm internals); it is used purely for focus + a stable test selector and is never read for content. So the production workspace can now stamp the renderer-neutral `[data-relayterm-terminal-input]` marker on a wterm mount (`data-renderer-input="marked"`), and the renderer-fair Path A / Path C input seam (`apps/web/e2e/SMOKE.md` § "Renderer-fair input") is available for a future wterm production-shell smoke. This **unblocks** — but does **not itself perform** — that smoke: no renderer-fair wterm matrix run has been done yet, and it stays a separate, deliberate slice. `ResttyRenderer` still lacks `focusTarget()`; the xterm and ghostty-web adapters already meet it.

**Promotion impact.** wterm clearing the gate (clean mount + functional render) is one data point, **not** a Gate 1 pass. With `focusTarget()` now implemented, the renderer-fairness *precondition* for grading the **Core-correctness** matrix is met — but Gate 1 still requires that matrix to actually be run and graded `works` / `works with caveats` renderer-fairly on the evaluated surface. That matrix run is a separate, deliberate smoke slice; it is not performed by the `focusTarget()` adapter slice and not authorised by this evaluation entry.

**Cross-links.** The gate-smoke evidence is [`docs/deployment/vps-staging-smoke.md`](../deployment/vps-staging-smoke.md) § "2026-05-14g · wterm production-shell renderer gate smoke"; the gates and the renderer-evaluation status are [`docs/terminal-renderer-evaluation.md`](../terminal-renderer-evaluation.md) § "Promotion criteria" and § "2026-05-14g · wterm production-shell renderer gate".

### Future work (explicit out-of-scope for this slice)

Production terminal UI; persistent per-renderer preference; renderer benchmarking harness; mobile/Tauri shell integration of the experimental renderer; jsdom/headless-browser verification of the real wterm WASM/DOM runtime; honoring the neutral cosmetic knobs (font, cursor, theme, scrollback) via wterm's CSS custom properties; surfacing wterm's `onTitle` channel; wiring wterm's `DebugAdapter` into the dev lab; the renderer-fair wterm production-shell matrix smoke that `focusTarget()` now unblocks (see "Production-shell evaluation status and `focusTarget()` caveat" above). Each is a separate, deliberate slice.
