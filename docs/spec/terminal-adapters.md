...
Production terminal UI; persistent per-renderer preference; renderer benchmarking harness; mobile/Tauri shell integration of the experimental renderer; jsdom/headless-browser verification of the real ghostty-web WASM runtime; a renderer-neutral resize/fit capability so the production "Fit" control is not xterm-specific (see "Production-shell evaluation status and resize/fit caveat" above). Each is a separate, deliberate slice.
