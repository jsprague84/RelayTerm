...
Defaults preserve baseline behaviour: `autofit` is `false` unless an operator opts in via Settings → "Fit terminal to its container", so fresh users see zero behaviour change and xterm's fixed-grid + manual-Fit posture is untouched on the default path. The implementation does **not** promote any renderer, does **not** change the xterm default, does **not** touch backend / protocol / session / orchestrator code, and does **not** change CSP or deploy config. The follow-on staging resmoke (`docs/wterm-fit-reflow-resmoke`) — re-running the wterm production-shell matrix with autofit enabled — is its own deliberate slice.
