...
**`focusTarget()` caveat (load-bearing).** `WtermRenderer` does **not** implement the optional `focusTarget()` method on the neutral `TerminalRenderer` interface, so the production workspace cannot stamp the renderer-neutral `[data-relayterm-terminal-input]` marker (`data-renderer-input="none"`) and the renderer-fair Path A / Path C input seam (`apps/web/e2e/SMOKE.md` § "Renderer-fair input") is unavailable for a wterm production-shell smoke. The `production-terminal-focus` button still focuses wterm's textarea-backed `InputHandler` (verified via `document.activeElement`), but a smoke cannot *verify* focus through the standard renderer-neutral selector. A future wterm production-shell evaluation that wants to grade Path A / Path C rows needs `focusTarget()` implemented first — the same precondition `ResttyRenderer` also lacks, and one the xterm and ghostty-web adapters already meet.

**Promotion impact.** wterm clearing the gate (clean mount + functional render) is one data point, **not** a Gate 1 pass. wterm cannot clear Gate 1 (experimental → production opt-in) on the evaluated surface yet because the Gate 1 bar is the full **Core-correctness** matrix graded `works` / `works with caveats`, and those rows cannot be *graded renderer-fairly* without `focusTarget()`. Implementing `focusTarget()` for `WtermRenderer` is the **precondition** for that grading — an adapter code slice, separate and deliberate, not itself the Gate 1 criterion and not authorised by this evaluation entry.
