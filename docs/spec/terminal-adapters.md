...
This section records what is **proven about ghostty-web on the production shell today**, and the load-bearing caveat the 2026-05-14 renderer-evaluation matrix smoke surfaced. It is the `docs/spec/terminal-adapters.md` half of the Gate 1 requirement in [`docs/terminal-renderer-evaluation.md`](../terminal-renderer-evaluation.md) § "Promotion criteria" — that a candidate's Core-correctness caveats "are documented in `docs/spec/terminal-adapters.md`".

**Current status (2026-05-14).** ghostty-web is **experimental and not promoted.** xterm remains the production compatibility baseline and the default renderer. ghostty-web mounts on the production shell ONLY when both of these hold:

- the operator has flipped the `experimentalRendererEvaluationEnabled` gate on in Settings AND selected `ghostty-web`, and
- the serving surface's CSP allows WASM compilation — `script-src` must include `'wasm-unsafe-eval'`. The staging surface carries that relaxation (host-side Traefik middleware, staging router only); the repo's production deploy templates stay strict, so ghostty-web does NOT mount on a default-strict production deployment. See [`docs/ghostty-web-wasm-csp.md`](../ghostty-web-wasm-csp.md) and the 2026-05-14 staging-smoke history in [`docs/deployment/vps-staging-smoke.md`](../deployment/vps-staging-smoke.md).

On the first graded production-shell matrix run (2026-05-14, staging surface) the **Core correctness** rows held — basic I/O, long output, trusted-paste through the production paste-safety pipeline, and detach/reconnect/replay all passed; unicode/box/wide output and the alternate-screen probe rendered correctly. That is one human-evaluator data point, not a promotion.

**Resize / fit / reflow caveat (load-bearing).** xterm.js carries the production baseline fit / resize behavior: `XtermRenderer` exposes a `fit()` helper (backed by `@xterm/addon-fit`) that computes the cell-grid dimensions from the container's pixel size, and the production "Fit" control drives it. `GhosttyWebRenderer` does **not** expose a `fit()` method — `fit()` is an xterm-specific capability, not part of the neutral `TerminalRenderer` surface. The production workspace's Fit control routes through `safeFit()` (`apps/web/src/lib/app/terminal/terminalLaunch.ts`), which probes for the `fit()` capability at runtime and is a clean no-op when the mounted renderer does not expose it. The 2026-05-14 matrix smoke observed, on the production shell:

- Clicking "Fit" after a browser-viewport resize left `stty size` unchanged — the runtime no-op path above, expected.
- ghostty-web did **not** reflow its cell grid to a narrowed container / browser viewport: it stayed at its mounted column count, and long lines clipped at the canvas edge rather than rewrapping to the narrower width. The terminal stayed usable — this was not a crash, a mount failure, or a redaction issue.

Treat the production "Fit" action as **xterm-specific** unless and until a renderer-neutral resize/fit capability is designed and implemented. This is **documented adapter behavior, not a `regression vs. baseline`** — the neutral interface never promised `fit()`, and the ghostty-web adapter does not claim it. The exact root cause inside ghostty-web / libghostty-vt (whether upstream's `Terminal` exposes a reflow-on-resize path the adapter has not wired, or libghostty-vt does not reflow on a column change at all) is **not** established by the smoke and is deliberately NOT asserted here — establishing it, and designing the fix, is its own slice (see "Future work" below).

**Promotion impact.**

- **Gate 1 (experimental → production opt-in)** requires every Core-correctness row to be `works` / `works with caveats` *with the caveats documented here* — the resize/fit/reflow caveat above is that documentation, so this caveat does not by itself block Gate 1. Resize/fit is a separate evaluation-matrix row, not a Core-correctness row (see [`docs/terminal-renderer-evaluation.md`](../terminal-renderer-evaluation.md) § "Evaluation matrix"). It does, however, leave an open decision the Gate 1 reviewer should make explicitly rather than let default silently: **either** implement renderer-neutral resize/fit support for ghostty-web, **or** record the non-reflow limitation as a knowingly accepted constraint of the experimental opt-in.
- **Gate 2 (production opt-in → production default)** requires resize / narrow-viewport parity with xterm, **or** an intentionally accepted product tradeoff written into the default-flip slice. A silent resize/reflow regression versus the xterm baseline is a Gate 2 blocker.

**Cross-links.** The matrix-smoke evidence is [`docs/deployment/vps-staging-smoke.md`](../deployment/vps-staging-smoke.md) § "2026-05-14e · Ghostty-web production-shell renderer matrix smoke"; the gates are [`docs/terminal-renderer-evaluation.md`](../terminal-renderer-evaluation.md) § "Promotion criteria"; the operator runbook is [`apps/web/e2e/SMOKE.md`](../../apps/web/e2e/SMOKE.md) § "D. Renderer evaluation smoke".

### Future work (explicit out-of-scope for this slice)

Production terminal UI; persistent per-renderer preference; renderer benchmarking harness; mobile/Tauri shell integration of the experimental renderer; jsdom/headless-browser verification of the real ghostty-web WASM runtime; implementing the renderer-neutral autofit capability for ghostty-web (the capability is designed in [`docs/renderer-neutral-autofit.md`](../renderer-neutral-autofit.md); ghostty-web accepts-and-no-ops it until it grows a container-fit path). Each is a separate, deliberate slice.
...
This section records what is **proven about restty on the production shell today**, and the load-bearing caveat the 2026-05-14 restty production-shell renderer gate smoke surfaced. restty is **experimental and not promoted.** xterm remains the production compatibility baseline and the default renderer.

**Current status (2026-05-14).** restty reaches the production shell only through the gated lazy loader (`rendererLoader.ts`), only when the operator has flipped the `experimentalRendererEvaluationEnabled` gate in Settings AND selected `restty`. On the 2026-05-14 staging-surface gate smoke, restty's **loader path was healthy** — the dynamic `import()` resolved, the `ResttyRenderer` constructor ran, `mount()` resolved, the WASM compiled under the staging surface's `script-src 'self' 'wasm-unsafe-eval'` CSP, and the backend session attached. The production workspace surfaced `data-renderer="restty"`, `data-renderer-experimental="true"`, `data-renderer-fallback=""` (no `adapter_mount_failed`), `data-phase="attached"`.

**Mounts-but-non-functional caveat (load-bearing).** Despite the clean mount diagnostics, restty rendered nothing usable on the production shell under the staging CSP:

- restty applies **inline styles** to size and position its DOM/canvas. With `style-src` unset, the CSP `default-src 'self'` fallback (no `'unsafe-inline'`) blocks every inline-style application from the restty chunk — so the restty `<canvas>` stayed at **1 × 1 px** and never sized to its container.
- restty's runtime text-shaper **`fetch()`es a font stack from `cdn.jsdelivr.net`** (Nerd Fonts, Noto Sans Symbols/CJK, emoji, Symbola). With `connect-src` unset, the same `default-src 'self'` fallback refuses every cross-origin font fetch.
- The headless evaluation browser additionally exposed **no WebGPU adapter** (`No available adapters`).

Net result: `last_seen_seq` stayed `0`, nothing rendered, and — because `mount()` *resolved* rather than rejected — the closed loader fallback taxonomy (`experimental_gate_off` / `unknown_renderer_id` / `adapter_load_failed` / `adapter_mount_failed`) **could not describe the failure**, so the production workspace showed **no operator-visible error panel**. This "mounted-but-non-functional" stage is distinct from ghostty-web's `adapter_mount_failed` (a `mount()` rejection). The exact restty-internal mechanism (which inline styles are load-bearing for canvas sizing, whether a self-hosted or bundled font stack would change behavior) is **not** established by the smoke and is deliberately NOT asserted here.

Additionally, `ResttyRenderer` does **not** implement the optional `focusTarget()` method on the neutral `TerminalRenderer` interface, so the production workspace cannot stamp the renderer-neutral `[data-relayterm-terminal-input]` marker (`data-renderer-input="none"`) and the renderer-fair Path A / Path C input seam is unavailable for a restty production-shell smoke. A future restty production-shell evaluation that wants to grade Path A / Path C rows needs `focusTarget()` implemented first (same precondition ghostty-web's adapter already meets).

**Promotion impact.** restty cannot clear Gate 1 (experimental → production opt-in) on the evaluated surface because the **Core-correctness** rows cannot be exercised — restty is not a usable renderer surface under a strict-`default-src` CSP. Whether to widen the staging CSP so restty can render at all (`style-src 'unsafe-inline'` for the inline-style block, plus a `connect-src` allowance or a self-hosted/bundled font stack for the jsdelivr block) is a **separate, deliberate later decision** — not authorised by this evaluation entry, and weighed against the same page-level-CSP tradeoffs documented for ghostty-web in [`docs/ghostty-web-wasm-csp.md`](../ghostty-web-wasm-csp.md). The production deploy templates remain strict.

**Cross-links.** The gate-smoke evidence is [`docs/deployment/vps-staging-smoke.md`](../deployment/vps-staging-smoke.md) § "2026-05-14f · restty production-shell renderer gate smoke"; the gates and the renderer-evaluation status are [`docs/terminal-renderer-evaluation.md`](../terminal-renderer-evaluation.md) § "Promotion criteria" and § "2026-05-14f · restty production-shell renderer gate".

### Future work (explicit out-of-scope for this slice)

Production terminal UI; persistent per-renderer preference; renderer benchmarking harness; mobile/Tauri shell integration of the experimental renderer; jsdom/headless-browser verification of the real restty WASM/WebGPU runtime; honoring the neutral cosmetic knobs (font, cursor, theme, scrollback) via `Restty`'s native APIs; restty pane / plugin / shader-stage surface integration; a renderer-neutral `focusTarget()` implementation for `ResttyRenderer` so a future production-shell smoke can drive renderer-fair input; the staging-CSP decision that would let restty render at all (see "Production-shell evaluation status and CSP caveat" above). Each is a separate, deliberate slice.
...
### Production-shell evaluation status and `focusTarget()` caveat

This section records what is **proven about wterm on the production shell today**, and the load-bearing caveat the 2026-05-14 wterm production-shell renderer gate smoke surfaced. wterm is **experimental and not promoted.** xterm remains the production compatibility baseline and the default renderer.

**Current status (2026-05-14).** wterm reaches the production shell only through the gated lazy loader (`rendererLoader.ts`), only when the operator has flipped the `experimentalRendererEvaluationEnabled` gate in Settings AND selected `wterm`. On the 2026-05-14 staging-surface gate smoke, wterm's **loader and mount path was healthy** — the dynamic `import()` resolved (lazy chunk, ~41 KB including `@wterm/core`'s inlined WASM), the `WtermRenderer` constructor ran, `mount()` resolved, the WASM compiled under the staging surface's `script-src 'self' 'wasm-unsafe-eval'` CSP, and the backend session attached. The production workspace surfaced `data-renderer="wterm"`, `data-renderer-experimental="true"`, `data-renderer-fallback=""` (no `adapter_mount_failed`), `data-phase="attached"`, with **zero console errors** during the mount.

**Mounts-and-renders caveat (the positive finding).** Unlike restty (which mounts but is non-functional under the same CSP), wterm is **visually and functionally healthy** on the production shell. wterm is DOM-rendered — the cell grid is ordinary DOM nodes, theming is via the `.wterm` host stylesheet that ships in the prod CSS bundle (static, not inline), and there is no canvas/WebGPU surface and no runtime font-CDN `fetch` — so none of restty's three compounding CSP failures apply. The `.wterm` DOM host sized correctly to its container (not a 1 × 1 wedge), and a diagnostic input probe confirmed the input → wire → output → render path works: trusted keystrokes reach wterm's textarea-backed `InputHandler`, round-trip through the wire, and render into the DOM grid.

**`focusTarget()` status (load-bearing).** `WtermRenderer` implements the optional `focusTarget()` method on the neutral `TerminalRenderer` interface (landed in `bde039e feat(web): expose wterm focus target`). It returns wterm's hidden keyboard `<textarea>` — the `InputHandler.textarea` element wterm appends to the host, attaches its `keydown` listener to, and the one `WTerm.focus()` ultimately focuses — and `null` before mount / after dispose / after a dispose that raced a pending `init()`. The adapter reaches that element through a narrow structural cast over wterm's type-level-private `input` / `textarea` fields, contained in the adapter (the only file in the repo that knows wterm internals); it is used purely for focus + a stable test selector and is never read for content. So the production workspace stamps the renderer-neutral `[data-relayterm-terminal-input]` marker on a wterm mount (`data-renderer-input="marked"`), and the renderer-fair Path A / Path C input seam (`apps/web/e2e/SMOKE.md` § "Renderer-fair input") is available for a wterm production-shell smoke. `ResttyRenderer` still lacks `focusTarget()`; the xterm and ghostty-web adapters also meet it.

**Production-shell matrix result (2026-05-14, staging surface).** With `focusTarget()` in place, the first **graded** wterm renderer-evaluation matrix ran on the production shell. The **Core correctness** rows held — basic I/O, long output, trusted-paste through wterm's DOM textarea `paste` handler into the production paste-safety pipeline, and detach/reconnect/replay all passed; unicode/box/wide-CJK output and the alternate-screen probe rendered correctly. That is one human-evaluator data point, **not** a promotion.

**Resize / fit / reflow caveat (load-bearing).** Like `GhosttyWebRenderer`, `WtermRenderer` does **not** expose a `fit()` method — `fit()` is an xterm-specific capability, not part of the neutral `TerminalRenderer` surface — so the production "Fit" control routes through `safeFit()` and is a clean runtime no-op on a wterm mount. Additionally, the wterm adapter defaults `wtermOnly.autoResize` to `false` (the caller drives sizing via `renderer.resize(cols, rows)` for cross-adapter parity), so wterm does **not** reflow its cell grid to a narrowed container / browser viewport on its own: the 2026-05-14 matrix smoke observed the `.wterm` DOM host pixel-width track the container while the cell grid / PTY geometry stayed at its mounted column count. The terminal stayed usable — not a crash, mount failure, or redaction issue. This is **documented adapter behavior, not a `regression vs. baseline`** — the neutral interface never promised `fit()` and the wterm adapter does not claim it. Treat the production "Fit" action as **xterm-specific** unless and until a renderer-neutral resize/fit capability is designed; opting wterm into its own `ResizeObserver`-driven auto-fit is possible via `wtermOnly.autoResize: true` but that is renderer-only escape-hatch behavior, not the neutral contract.

**Resize / fit / reflow — investigation findings (2026-05-14j, `feat/wterm-fit-reflow-investigation`).** A code-reading investigation of `@wterm/dom@0.2.1` (`WTerm`), `WtermRenderer`, `rendererLoader.ts`, and `settingsToRendererOptions` established the precise root cause. **It is not a wterm/upstream limitation.** wterm *is* capable of grid reflow:

- `WTerm.resize(cols, rows)` — public, and the adapter wires it through `WtermRenderer.resize()` — calls `bridge.resize(cols, rows)` **and** `renderer.setup(cols, rows)`, a genuine cell-grid + PTY-geometry reflow. Anything that recomputes `(cols, rows)` and calls `renderer.resize()` already reflows wterm correctly.
- wterm also ships a **native fit-to-container** path: with `autoResize: true` (wterm's own default), `init()` runs `_setupResizeObserver()` — it measures char/row pixel size and attaches a `ResizeObserver` to the host that, on every container resize, recomputes `cols = floor(width/charWidth)` / `rows = floor(height/rowHeight)` and calls `resize()`, which fans out through `onResize`. With `autoResize: false`, `init()` runs `_lockHeight()` instead — it pins the host's pixel height and never observes the container.

The non-reflow seen on the production shell is therefore the sum of **three RelayTerm-side facts**, not an upstream gap:

1. `WtermRenderer` defaults `autoResize` to `false` (a deliberate "caller drives sizing" parity choice with xterm/ghostty-web/restty) — so wterm's native `ResizeObserver` fit is disabled and `_lockHeight()` runs.
2. The `wtermOnly.autoResize` opt-in is **structurally unreachable from the production shell**: `settingsToRendererOptions()` returns `Required<BaseTerminalRendererOptions>`, and `wtermOnly` lives on `WtermRendererOptions`, not on the base type; `rendererLoader.ts` only forwards the base options plus `cols`/`rows`. So in production wterm's `autoResize` is *always* `false`. (The dev lab does not set it either.) The "opt back in via `wtermOnly.autoResize: true`" note above is a dev-lab-only escape hatch today, not a production-shell affordance.
3. The production "Fit" affordance and `safeFit()` are **xterm-`FitAddon`-shaped**: they duck-type for a synchronous `fit(): { cols, rows }` method. wterm exposes no `fit()` and no *public* one-shot container-measurement method (`_measureCharSize`, `_container`, `_setupResizeObserver`, `_lockHeight` are all private), so `safeFit()` finds nothing and no-ops.

`safeFit()` *is* too xterm-shaped (this was an open investigation question). The synchronous one-shot `fit(): { cols, rows }` contract is the `FitAddon` contract; wterm's honest fit model is **observer-driven, not a synchronous one-shot** — the `ResizeObserver` callback fires asynchronously (the char-size measurement *inside* the callback is itself synchronous DOM measurement), so there is no method a caller can invoke to get post-fit dims back inline. That is a different *shape*, not just a different implementation. A single neutral `fit()` cannot unify the two without leaking one renderer's model onto the other.

**No safe in-boundary code change was found for this investigation slice**, and the options were rejected for concrete reasons:

- *Implement a synchronous `fit()` on `WtermRenderer`.* Would require reimplementing wterm's private `_measureCharSize` against its private `_container` DOM and CSS class names — a fragile internals leak far beyond the single narrow `focusTarget()` structural cast the adapter already contains.
- *Add an optional `fit?()` to `terminal-core`.* Would formalize xterm's synchronous shape without giving wterm (or ghostty-web) a way to honor it — it does not fix wterm and risks an xterm-shaped abstraction leak into the neutral surface.
- *Flip the adapter `autoResize` default to `true`, or route `wtermOnly` through `settingsToRendererOptions` / the production shell.* Behavior + production-surface changes outside an investigation slice's boundary; `wtermOnly` is also a deliberately non-portable escape hatch that must not be promoted into the persisted preference model.

**Recommendation.** The renderer-neutral resize/fit capability already named as future work below should be designed **observer-shaped** — an optional "observe my container and self-fit, emitting `onResize`" capability — rather than xterm's synchronous one-shot `fit()`. xterm's `FitAddon` can be wrapped to that shape; wterm's `autoResize` `ResizeObserver` *already is* that shape; ghostty-web can no-op until it grows one. That design — together with the product decision of whether the production shell / Settings expose an "auto-fit" toggle — is its own deliberate slice, with a follow-on staging resmoke (`docs/wterm-fit-reflow-resmoke`) once behavior actually changes. This investigation changed **no** adapter, `terminal-core`, production-shell, protocol, backend, CI, deploy, or CSP code — it is docs-only.

**Design status (since 2026-05-14, `docs/renderer-neutral-autofit-design`).** That observer-shaped design has now landed as a docs-only design slice: [`docs/renderer-neutral-autofit.md`](../renderer-neutral-autofit.md). The chosen design is a mount-time renderer-neutral `autofit` option on `BaseTerminalRendererOptions` plus an optional `autofitActive()` query on `TerminalRenderer` — xterm wires it via `ResizeObserver` + `FitAddon`, wterm maps it to `WTerm.autoResize`, ghostty-web / restty accept-and-no-op it honestly.

**Implementation status (since 2026-05-15, `feat/renderer-neutral-autofit`).** The design has landed in code. The shared interface in `@relayterm/terminal-core` carries `BaseTerminalRendererOptions.autofit?: boolean` (default `false`) and `TerminalRenderer.autofitActive?(): boolean`. Per-adapter contract:

- **`XtermRenderer`** — `autofit: true` installs an adapter-owned `ResizeObserver` on the mount element; each container change calls `FitAddon.fit()` (rAF-coalesced), fanning through the existing `onResize` seam. `autofitActive()` returns `true` while the observer is live, `false` pre-mount / post-dispose / when `autofit` is `false` / in an environment without `ResizeObserver`. Late observer callbacks are guarded by `#disposed` and a `cancelAnimationFrame` on dispose, so no fit fires post-tear-down. The xterm-only one-shot `fit()` is **unchanged** and still drives the manual button when autofit is inactive.
- **`WtermRenderer`** — `toWtermOptions` resolves `autoResize` with three layers of precedence (mount-time, not runtime): `wtermOnly.autoResize` (explicit escape hatch — wins when set, in either direction); else `options.autofit`; else adapter default `false`. `autofitActive()` returns the resolved `autoResize` value while the renderer is mounted, `false` otherwise (pre-mount, post-dispose, failed `init()`, dispose-during-`init()`). wterm's own `ResizeObserver` does the fit work and fans through `onResize` — no RelayTerm-side reimplementation of wterm's private `_measureCharSize` / `_container` measurement.
- **`GhosttyWebRenderer`** — accepts `autofit` on the neutral surface and silently drops it during option mapping (same pattern as the cosmetic knobs it cannot honour); `autofitActive()` returns `false` honestly. The workspace mirrors this onto `data-renderer-autofit="unsupported"` when the operator enabled autofit and `"off"` otherwise. Revisited when / if ghostty-web grows a container-observation path upstream.
- **`ResttyRenderer`** — same accept-and-drop shape as ghostty-web; `autofitActive()` returns `false`. restty is independently CSP / WebGPU / `focusTarget()` gated, so autofit is moot here until that viability decision lands.

Defaults preserve baseline behaviour: `autofit` is `false` unless an operator opts in via Settings → "Fit terminal to its container", so fresh users see zero behaviour change and xterm's fixed-grid + manual-Fit posture is untouched on the default path. The implementation does **not** promote any renderer, does **not** change the xterm default, does **not** touch backend / protocol / session / orchestrator code, and does **not** change CSP or deploy config.

**Staging resmoke landed (2026-05-15, `docs/wterm-fit-reflow-resmoke`).** The follow-on staging resmoke ran on the production shell against the freshly-recreated `:main` web image. Under operator opt-in (`autofitEnabled = true`), wterm's PTY genuinely reflows on container resize: at 1440 × 900 the container is 896 × 448 px and `stty size = 24 80`; narrowing to 390 × 844 collapses `.wterm` to 327 × 448 px and reflows `stty size` to **`24 35`**; restoring to 1440 × 900 settles at `24 103`. The 2026-05-14i baseline (autofit not yet shipped) stayed at `24 80` for the same step. xterm's autofit also reflows end to end (`24 80` → `26 40` at 390 × 844). The renderer-neutral autofit capability does what the design said it would; the resize/fit caveat the 14j investigation opened is **substantively closed** for both renderers under the operator-opt-in path. The resmoke also discovered a workspace-side Svelte 5 reactivity bug — `let renderer` is not `$state`, so the `data-renderer-autofit = $derived(computeRendererAutofitStatus({ autofitEnabled, renderer }))` derivation never re-runs after the live renderer assigns and the attribute stays at `"unsupported"` for both wterm AND xterm even when the underlying autofit IS active. The Fit-button autofit-active tooltip never appears for the same reason. The bug only affects the workspace diagnostic surface, not the underlying autofit capability. Follow-on fix slice **`fix(web): update autofit diagnostic after renderer mount`** landed as `79c216b` (makes the workspace `renderer` reference reactive via `$state.raw`, so the `autofitStatus` derivation re-runs on the post-mount `renderer = r` assignment) and was verified end-to-end on the production shell by the 2026-05-15b diagnostic resmoke — `data-renderer-autofit` now lands on `"active"` post-mount for both wterm AND xterm under `autofitEnabled = true`, and the Fit-button autofit-active tooltip ("Autofit is keeping the terminal sized to its container.") finally renders. Detail in [`docs/deployment/vps-staging-smoke.md`](../deployment/vps-staging-smoke.md) §§ "2026-05-15 · wterm production-shell renderer-neutral autofit resmoke" and "2026-05-15b · wterm autofit diagnostic resmoke".

**Detach / reattach caveat.** On reattach the wterm adapter is a fresh mount: the 2026-05-14 matrix smoke observed the remounted `.wterm` DOM grid empty until new output arrived. Wire-side replay is correct (same session row, still active); renderer-side scrollback parity across reattach is a separate property the neutral interface does not promise — the same posture the xterm baseline records.

**Promotion impact.** wterm clearing the gate (clean mount + functional render) and completing one graded matrix run are human-evaluator data points, **not** a Gate 1 pass. Gate 1 (experimental → production opt-in) requires every Core-correctness row to be `works` / `works with caveats` *with the caveats documented here* — the resize/fit/reflow and detach/reattach caveats above are that documentation, so they do not by itself block Gate 1; resize/fit is a separate evaluation-matrix row, not a Core-correctness row. As with ghostty-web, this leaves an open decision a Gate 1 reviewer should make explicitly rather than let default silently: **either** implement renderer-neutral resize/fit support, **or** record the non-reflow limitation as a knowingly accepted constraint of the experimental opt-in. Gate 2 (production opt-in → production default) still requires resize / narrow-viewport parity with xterm or an intentionally accepted product tradeoff written into the default-flip slice.

**Cross-links.** The gate-smoke evidence is [`docs/deployment/vps-staging-smoke.md`](../deployment/vps-staging-smoke.md) § "2026-05-14g · wterm production-shell renderer gate smoke"; the matrix-smoke evidence is § "2026-05-14i · wterm production-shell renderer matrix smoke" in the same file; the gates and the renderer-evaluation status are [`docs/terminal-renderer-evaluation.md`](../terminal-renderer-evaluation.md) § "Promotion criteria", § "2026-05-14g · wterm production-shell renderer gate", and § "2026-05-14i · wterm production-shell renderer matrix smoke"; the operator runbook is [`apps/web/e2e/SMOKE.md`](../../apps/web/e2e/SMOKE.md) § "D. Renderer evaluation smoke".

### Future work (explicit out-of-scope for this slice)

Production terminal UI; persistent per-renderer preference; renderer benchmarking harness; mobile/Tauri shell integration of the experimental renderer (the **plan** for evaluating wterm on mobile / WebView surfaces — Android Chrome first, then Android Tauri WebView — has now landed in [`docs/wterm-mobile-smoke-plan.md`](../wterm-mobile-smoke-plan.md); the smoke itself has not yet run); jsdom/headless-browser verification of the real wterm WASM/DOM runtime; honoring the neutral cosmetic knobs (font, cursor, theme, scrollback) via wterm's CSS custom properties; surfacing wterm's `onTitle` channel; wiring wterm's `DebugAdapter` into the dev lab; implementing the renderer-neutral autofit capability (designed in [`docs/renderer-neutral-autofit.md`](../renderer-neutral-autofit.md) — wterm maps it to `WTerm.autoResize`) so the production shell can keep wterm fitted to its container. Each is a separate, deliberate slice.
