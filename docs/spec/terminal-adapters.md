...
**Design status (since 2026-05-14, `docs/renderer-neutral-autofit-design`).** That observer-shaped design has now landed as a docs-only design slice: [`docs/renderer-neutral-autofit.md`](../renderer-neutral-autofit.md). The chosen design is a mount-time renderer-neutral `autofit` option on `BaseTerminalRendererOptions` plus an optional `autofitActive()` query on `TerminalRenderer` — xterm wires it via `ResizeObserver` + `FitAddon`, wterm maps it to `WTerm.autoResize`, ghostty-web / restty accept-and-no-op it honestly. Implementation is the named follow-on slice `feat/renderer-neutral-autofit`; the per-adapter contract here is updated to the implemented shape when that slice lands.
